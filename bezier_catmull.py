# Catmull-Rom implementation starts at line 248
# By Amir Eskandari
'''
...
# Catmull-Rom Spline ---------------------------------
#  the derivatives for the very first and last control points is considered zero

def catmull_rom_spline(p0, p1, p2, p3, t):
    # Basis 
    b0 = -t/2 + pow(t,2) - (1/2)*pow(t,3)
    b1 = 1 - (5/2)*pow(t,2) + (3/2)*pow(t,3)
    b2 = t/2 + (2)*pow(t,2) - (3/2)*pow(t,3)
    b3 = -(1/2)*pow(t,2) + (1/2)*pow(t,3)
    result = p0*b0 + p1*b1 + p2*b2 + p3*b3
    return result

def gen_catmull_rom_curve_points(key_points):
    key_number = len(key_points)
    if key_number < 4:
        return
    step_size = 0.01
    ...
    for i in range(0, segment_number):
        ...
        for s in range(0, step_count+1):
            t = s * step_size
            one_position_on_curve = catmull_rom_spline(p0, p1, p2, p3, t)
            positions_on_curve.append(one_position_on_curve)
    return positions_on_curve
...
while (
    ...
    if len(key_points) > 0:
        ...
        # positions_on_curve = gen_catmull_rom_curve_points(Point3.num_list_to_point_list(key_points))
